...
- If Vercel shows **`MIDDLEWARE_INVOCATION_FAILED`**, it was often `Headers#getSetCookie` missing on Edge (Neon Auth calls it on `fetch` responses). The app polyfills it in `src/middleware.ts` via `ensureHeadersGetSetCookie()`. Also confirm **`NEON_AUTH_BASE_URL`** and **`NEON_AUTH_COOKIE_SECRET`** (≥32 chars) are set for **Production** (and Preview) on Vercel.
